...
const {
  ...
  ArgumentError
} = require('common-errors'); // lodash fp
...
const set = require('lodash/set'); // local deps
...
const {
  kReplyHeaders
} = require('./constants'); // serialization functions


const {
  jsonSerializer,
  jsonDeserializer
} = require('./utils/serialization'); // cache references
...
const toUniqueStringArray = routes => Array.isArray(routes) ? uniq(routes) : [routes];
/**
...
const initRoutingFn = (messageHandler, transport) => {
  ...
  function responseHandler(raw, error, data) {
    ...
    return !properties.replyTo || !properties.correlationId ? transport.noop(error, data, span, raw) : transport.reply(properties, {
      error,
      data
    }, span, raw);
  }
  ...
  return function router(message, properties, raw) {
    // add instrumentation
    const appId = safeJSONParse(properties.appId, this.log); // opentracing instrumentation
...
class AMQPTransport extends EventEmitter {
  ...
  constructor(opts = {}) {
    ...
    _defineProperty(this, "_onConsume", _router => {
      assert(is.fn(_router), '`router` must be a function'); // use bind as it is now fast
      ...
      return async function consumeMessage(incoming) {
        ...
        amqpTransport.emit('pre', incoming); // extract message data
        ...
        const {
          contentType,
          contentEncoding
        } = properties; // parsed input data

        const message = await parseInput(incoming.raw, contentType, contentEncoding); // useful message properties

        const props = extend({}, properties, pick(incoming, AMQPTransport.extendMessageProperties)); // pass to the consumer message router
    ...
    _defineProperty(this, "_on406", (params, err) => {
      this.log.warn({
        params
      }, '[406] error declaring exchange/queue:', err.replyText);
    });

    _defineProperty(this, "_onConnect", () => {
      const {
        serverProperties
      } = this._amqp;
      const {
        ...
        version
      } = serverProperties; // emit connect event through log

      this.log.info('connected to %s v%s', clusterName, version); // https://github.com/dropbox/amqp-coffee#reconnect-flow
    ...
    _defineProperty(this, "_onClose", err => {
      // emit connect event through log
      this.log.warn({
        err
      }, 'connection is closed'); // re-emit close event
    ...
    const config = this.config = validateResult.value; // prepares logger

    this.log = loggerUtils.prepareLogger(config); // init cache or pass-through operations

    this.cache = new Cache(config.cache); // reply storage, where we'd save correlation ids
    // and callbacks to be called once we are done

    this.replyStorage = new ReplyStorage(); // delay settings for reconnect

    this.recovery = new Backoff(config.recovery); // init open tracer - default one is noop

    this.tracer = config.tracer || new opentracing.Tracer(); // setup instance

    this._replyTo = null;
    this._consumers = new Map();
    this._queues = new WeakMap();
    this._reconnectionHandlers = new WeakMap();
    this._boundEmit = this.emit.bind(this); // Form app id string for debugging
    ...
    this._defaultOpts = _objectSpread({}, config.defaultOpts);
    this._defaultOpts.appId = this._appIDString;
    this._extraQueueOptions = {}; // DLX config
    ...
    if (config.dlx.enabled === true) {
      ...
      this._extraQueueOptions.arguments = {
        'x-dead-letter-exchange': config.dlx.params.exchange
      };
    }
  }
  /**
  ...
  async createQueue(opts) {
    ...
    const userParams = is.string(opts) ? {
      queue: opts
    } : opts;
    const requestedName = userParams.queue;
    const params = merge({
      autoDelete: !requestedName,
      durable: !!requestedName
    }, userParams);
    log.debug({
      params
    }, 'initializing queue');
    const queue = ctx.queue = await amqp.queueAsync(params);
    await Promise.resolve(queue.declareAsync()).catch(AMQPTransport.error406, this._on406.bind(this, params)).tapCatch(err => {
      log.warn({
        err,
        queue: params.queue
    ...
    const options = ctx.options = _objectSpread({}, ctx.queue.queueOptions);
    ...
    log.info({
      queue: options.queue
    }, 'consumer is being created'); // setup consumer
  ...
  handlePrivateConsumerError(consumer, queue, err) {
    ...
    if (error && error.replyCode === 404 && error.replyText.indexOf(queue) !== -1) {
      ...
      this.log.error('consumer returned 404 error', error); // reset replyTo queue and ignore all future errors

      consumer.removeAllListeners('error');
      consumer.removeAllListeners('cancel');
      consumer.on('error', noop);
      consumer.close(); // recreate queue
  ...
  async handleConsumerError(consumer, queue, err, res) {
    const error = err.error || err; // https://www.rabbitmq.com/amqp-0-9-1-reference.html -
  ...
  async rebindConsumer(consumer, err, res) {
    const msg = err ? err.replyText : 'uncertain'; // cleanup a bit

    this.log.warn({
      err,
      res
    }, 're-establishing connection after %s', msg); // saved reference to re-establishing function
    ...
    if (establishConsumer == null) {
      this.log.fatal({
        err,
        res
      }, 'failed to fetch connection handler');
      return;
    }
  ...
  handlePrivateConsumerCancel(consumer) {
    ...
    consumer.removeAllListeners('cancel');
    consumer.on('error', noop);
    consumer.close(); // recreate queue unless it is already being recreated
  ...
  async createPrivateQueue(attempt = 0) {
    ...
    const queueOpts = _objectSpread({}, this.config.privateQueueOpts, {
    ...
    try {
      const {
        ...
        queue,
        options
      } = createdQueue = await this.createQueue(queueOpts); // remove existing listeners

      consumer.removeAllListeners('error');
      consumer.removeAllListeners('cancel'); // consume errors - re-create when we encounter 404 or on cancel

      consumer.on('error', this.handlePrivateConsumerError.bind(this, consumer, options.queue));
      consumer.once('cancel', this.handlePrivateConsumerCancel.bind(this, consumer)); // declare _replyTo queueName

      this._replyTo = options.queue; // bind temporary queue to headers exchange for DLX messages
  ...
  async bindQueueToExchangeOnRoutes(routes, queue, oldQueue = Object.create(null)) {
    ...
    const work = [this.bindExchange(queue, rebindRoutes, this.config.exchangeArgs)]; // bind same queue to headers exchange
  ...
  async createConsumedQueue(messageHandler, listen = [], options = {}) {
    ...
    if (config.bindPersistantQueueToHeadersExchange === true) {
      for (const route of listen.values()) {
        assert.ok(/^[^*#]+$/, route, 'with bindPersistantQueueToHeadersExchange: true routes must not have patterns');
      }
    ...
    const establishConsumer = async (attempt = 0) => {
      ...
      const oldQueue = queues.get(establishConsumer); // if we have old consumer
      ...
      try {
        const {
          queue
        } = createdQueue = await this.createQueue(_objectSpread({}, queueOptions));
      ...
      const {
        consumer,
        queue
      } = createdQueue; // save ref to WeakMap

      consumers.set(establishConsumer, consumer);
      queues.set(establishConsumer, queue);
      connectionHandlers.set(consumer, establishConsumer); // remove previous listeners if we re-use the channel
      // for any reason

      consumer.removeAllListeners('error');
      consumer.removeAllListeners('cancel');
      consumer.on('error', this.handleConsumerError.bind(this, consumer, queue));
      consumer.on('cancel', this.rebindConsumer.bind(this, consumer)); // emit event that we consumer & queue is ready

      const queueName = queue.queueOptions.queue;
      log.info({
        queueName,
        consumerTag: consumer.consumerTag
      }, 'consumed-queue-reconnected');
    ...
    }, 'creating consumed queue');
    const queueName = await establishConsumer();
    this.log.debug({
      listen,
      queue: queueName
    }, 'bound `ready` to establishConsumer');
  ...
  async closeConsumer(consumer) {
    this.log.warn('closing consumer', consumer.consumerTag); // cleanup after one-self
    ...
    this._boundEmit('consumer-close', consumer); // close channel


    await Promise.fromCallback(done => {
      consumer.cancel(done);
    }).timeout(5000).catch(Promise.TimeoutError, noop);
    this.log.info({
      consumerTag: consumer.consumerTag
    }, 'closed consumer');
  }
  ...
  async stopConsumedQueue(consumer) {
    ...
    this.log.debug({
      establishConsumer: !!establishConsumer
    }, 'fetched establish consumer');
  ...
  declareExchange(params) {
    return this._amqp.exchangeAsync(params).call('declareAsync').catch(AMQPTransport.error406, this._on406.bind(this, params));
  }
  /**
  ...
  bindExchange(queue, _routes, opts = {}) {
    ...
    const routes = toUniqueStringArray(_routes); // default params
    ...
    return this.declareExchange(params).return(routes).map(route => this.bindRoute(exchange, queue, route));
  }
  /**
  ...
  bindHeadersExchange(queue, _routes, opts, headerName = true) {
    ...
    return this.declareExchange(params).return(routes).map(route => {
  ...
  unbindExchange(queue, _routes) {
    ...
    return Promise.map(routes, route => queue.unbindAsync(exchange, route).tap(() => {
      const queueName = queue.queueOptions.queue;
  ...
  async sendToServer(exchange, queueOrRoute, _message, options) {
    ...
    const message = options.skipSerialize === true ? _message : await serialize(_message, publishOptions);
    const request = await this._amqp.publishAsync(exchange, queueOrRoute, message, publishOptions); // emit original message
  ...
  publish(route, message, options = {}, parentSpan) {
    ...
    const exchange = is.string(options.exchange) ? options.exchange : this.config.exchange;
    span.addTags({
      [Tags.SPAN_KIND]: Tags.SPAN_KIND_MESSAGING_PRODUCER,
      [Tags.MESSAGE_BUS_DESTINATION]: `${exchange}:${route}`
    });
    return wrapPromise(span, this.sendToServer(exchange, route, message, options));
  }
  /**
  ...
  send(queue, message, options = {}, parentSpan) {
    ...
    const exchange = is.string(options.exchange) ? options.exchange : '';
    span.addTags({
      [Tags.SPAN_KIND]: Tags.SPAN_KIND_MESSAGING_PRODUCER,
      [Tags.MESSAGE_BUS_DESTINATION]: `${exchange || '<empty>'}:${queue}`
    });
    return wrapPromise(span, this.sendToServer(exchange, queue, message, options));
  }
  /**
  ...
  publishAndWait(route, message, options = {}, parentSpan) {
    ...
    return wrapPromise(span, this.createMessageHandler(route, message, options, this.publish, span));
  }
  /**
  ...
  sendAndWait(queue, message, options = {}, parentSpan) {
    ...
    return wrapPromise(span, this.createMessageHandler(queue, message, options, this.send));
  }
  /**
  ...
  _publishOptions(options = {}) {
    ...
    const {
      ...
      gzip: needsGzip
          ...
          opts = _objectWithoutProperties(options, ["skipSerialize", "gzip"]); // force contentEncoding
    ...
    defaults(opts, this._defaultOpts); // append request timeout in headers
  ...
  reply(properties, message, span, raw) {
    ...
    if (raw !== undefined) {
      promise = promise.finally(() => this.emit('after', raw));
    }

    return span === undefined ? promise : wrapPromise(span, promise);
  }
  /**
  ...
  async createMessageHandler(routing, message, options, publishMessage, span) {
    ...
    const replyOptions = this._replyOptions(options); // ensure that reply queue exists before sending request
    ...
    const {
      replyStorage
    } = this; // generate response id

    const correlationId = options.correlationId || uuid.v4(); // timeout before RPC times out

    const timeout = options.timeout || this.config.timeout; // slightly longer timeout, if message was not consumed in time, it will return with expiration
    ...
    this.log.trace('message pushed into reply queue in %s', latency(time)); // add custom header for routing over amq.headers exchange

    set(options, 'headers.reply-to', replyTo); // add opentracing instrumentation
    ...
    publishMessage.call(this, routing, message, _objectSpread({}, options, {
      replyTo,
      correlationId,
  ...
  /* , raw */
    ...
    // if private queue has nack set - we must ack msg
    const {
      correlationId,
      replyTo,
      headers
    ...
    const {
      'x-death': xDeath
    } = headers; // retrieve promised message

    const future = this.replyStorage.pop(correlationId); // case 1 - for some reason there is no saved reference, example - crashed process
    ...
    this.log.trace('response returned in %s', latency(future.time)); // if message was dead-lettered - reject with an error
  ...
  async _parseInput(_data, contentType = 'application/json', contentEncoding = 'plain') {
    ...
    switch (contentEncoding) {
      case 'gzip':
        data = await gunzip(_data).catchReturn({
          err: PARSE_ERR
        });
        break;
...
_defineProperty(AMQPTransport, "extendMessageProperties", ['deliveryTag', 'redelivered', 'exchange', 'routingKey', 'weight']);

_defineProperty(AMQPTransport, "error406", {
  replyCode: 406
});
