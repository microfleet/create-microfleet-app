...
exports = module.exports = AMQPTransport => {
  ...
  AMQPTransport.create = function create(_config, _messageHandler) {
    ...
    const amqp = new AMQPTransport(config); // connect & resolve AMQP connector & message handler if it exists
  ...
  AMQPTransport.connect = function connect(config, _messageHandler, _opts = {}) {
    return AMQPTransport.create(config, _messageHandler).spread(async (amqp, messageHandler) => {
      // do not init queues
  ...
  AMQPTransport.multiConnect = function multiConnect(config, _messageHandler, opts = []) {
    return AMQPTransport.create(config, _messageHandler).spread(async (amqp, messageHandler) => {
      ...
      await Promise.map(amqp.config.listen, (route, idx) => {
        ...
        const queueName = config.queue ? `${config.queue}-${route.replace(/[#*]/g, '.')}` : config.queue;
...
const copyErrorData = ['code', 'name', 'errors', 'field', 'reason', 'stack'];
/**
