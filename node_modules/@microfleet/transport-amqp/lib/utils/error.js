...
const Errors = require('common-errors'); // error generator
  ...
  generateMessage() {
    ...
    this.xDeath.forEach(rejectionEntry => {
      switch (rejectionEntry.reason) {
        ...
        case 'expired':
          message.push(`Expired from queue "${rejectionEntry.queue}" with routing keys ` + `${JSON.stringify(rejectionEntry['routing-keys'])} ` + `after ${rejectionEntry['original-expiration']}ms ${rejectionEntry.count} time(s)`);
          break;
