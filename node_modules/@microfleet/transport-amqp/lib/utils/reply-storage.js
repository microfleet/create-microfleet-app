...
class ReplyStorage {
  ...
  onTimeout(correlationId) {
    ...
    const {
      ...
      routing,
      timeout
    } = storage.get(correlationId); // clean-up

    storage.delete(correlationId); // reject with a timeout error
  ...
  reject(correlationId, error) {
    ...
    const {
      timer,
      reject
    } = storage.get(correlationId); // remove timer

    clearTimeout(timer); // remove reference

    storage.delete(correlationId); // now resolve promise and return an error

    setImmediate(reject, error);
  }

  pop(correlationId) {
    const future = this.storage.get(correlationId); // if undefind - early return
    ...
    clearTimeout(future.timer); // remove reference to it

    this.storage.delete(correlationId); // return data
